...
export default function CqPenalitesPage() {
  ...
  // 🛡️ L'FIX: Helper pour l'affichage en négatif
      ...
      Promise.all([
        ...
        const mappedErrs: UnifiedPenalty[] = errs.map(e => {
          ...
          return {
            ...
            impact: Math.abs(e.impactEstime || 0), // 🛡️ Toujours positif pour le calcul
            statut: st
          };
        });
        ...
        const mapCq = (data: any[], type: string): UnifiedPenalty[] => data.map(c => {
          ...
          return {
            ...
            impact: Math.abs(c.mtSst || c.montant || 0), // 🛡️ Toujours positif pour le calcul
            statut: st
          };
        });
  ...
  const handleExportBilan = () => {
    ...
    const dataToExport = unifiedData.map(item => ({
      ...
      'Impact Financier (€)': -Math.abs(item.impact), // 🛡️ Négatif dans Excel
    ...
    dataToExport.push({
      ...
      'Impact Financier (€)': -Math.abs(totalPenalties),
      'Statut Actuel': ''
    });
      ...
      <div className={styles.container}>
        ...
        <div className={styles.detailsSection}>
          <div className={styles.leftColumn}>
            <h2>Registre Unifié des Pénalités</h2>
            <div className={styles.tableWrapper}>
              {loading ? (
                ...
                <table className={styles.table}>
                  <thead>
                    <tr><th>Type</th><th>Dossier / Réf</th><th>Motif / Règle</th><th>Impact (€)</th><th>Statut</th></tr>
                  </thead>
                  <tbody>
                    {unifiedData.map((item, index) => (
                      <tr key={item.id} className={styles.tableRow} style={{ animationDelay: `${index * 0.03}s` }}>
                        <td><span className={`${styles.typeBadge} ${item.source === 'ERREUR' ? styles.typeErreur : styles.typeCq}`}>{item.type}</span></td>
                        <td className={styles.reference}>{item.reference}</td>
                        <td style={{ color: '#475569', fontWeight: '700' }}>{item.description}</td>
                        <td className={styles.amount}>{formatPenalty(item.impact)}</td>
                        <td>
                          <span className={`${styles.statutBadge} ${item.statut === 'NON_CONTESTE' ? styles.statutNonConteste : item.statut === 'EN_ATTENTE' ? styles.statutEnAttente : item.statut === 'ACCEPTE' ? styles.statutAccepte : styles.statutRefuse}`}>
            ...
            <InteractiveCard delayIndex={1}>
              <div className={styles.impactVault}>
                ...
                <div className={styles.vaultMain}>
                  ...
                  <h2 className={styles.vaultAmount}>{formatPenalty(totalPenalties)}</h2>
                  <span className={styles.vaultSub}>Pour le mois {month}/{year}</span>
                </div>
                <div className={styles.projectionRow}>
                  <span>Pénalités Qualité</span>
                  <strong style={{ color: '#ef4444' }}>{formatPenalty(totalPenalties)}</strong>
                </div>
                <button className={styles.detailsBtn} onClick={handleExportBilan}>Télécharger le Bilan (Excel)</button>
              </div>
            </InteractiveCard>
          </div>
        </div>
